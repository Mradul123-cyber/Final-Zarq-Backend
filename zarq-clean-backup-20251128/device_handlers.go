...
func sendMessageHandler(w http.ResponseWriter, r *http.Request) {
	...
	// ADMIN-ONLY GROUP CHECK: For specific groups, only admins/owners can send messages
	if req.ConversationID == 46 { // Chess Tournament group
		var userRole string
		err = tx.QueryRow(`
			SELECT role FROM conversation_members 
			WHERE conversation_id = $1 AND profile_uid = $2
		`, req.ConversationID, senderUID).Scan(&userRole)
		if err != nil {
			log.Printf("sendMessageHandler: check user role error: %v", err)
			http.Error(w, "db error", http.StatusInternalServerError)
			return
		}
		if userRole != "owner" && userRole != "admin" {
			log.Printf("sendMessageHandler: Non-admin user %s attempted to send message in admin-only group %d", senderUID, req.ConversationID)
			http.Error(w, "Only admins can send messages in this group", http.StatusForbidden)
			return
		}
	}
	// 2) Insert message
