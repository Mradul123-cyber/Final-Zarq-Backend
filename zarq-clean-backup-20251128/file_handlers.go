...
// Upload avatar handler - simple unencrypted upload for profile/group avatars
func uploadAvatarHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Auth
	_, _, err := getVerifiedToken(r)
	if err != nil {
		http.Error(w, "Unauthorized", http.StatusUnauthorized)
		return
	}

	// Parse multipart form (max 5MB for avatars)
	...
	if !contains(allowedImageTypes, mimeType) {
	...
	if header.Size > 5*1024*1024 {
		http.Error(w, "File too large. Max 5MB for avatars.", http.StatusBadRequest)
		return
	}
	...
	// Return URL
	avatarURL := fmt.Sprintf("http://192.168.29.81:8080/uploads/avatars/%s", filename)
